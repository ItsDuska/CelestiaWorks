...
namespace celestia
{
	...
	LRESULT CALLBACK Win32Window::windowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_NCCREATE:
			...
			Win32Window* windowPtr = (Win32Window*)pcs->lpCreateParams;
	...
	Win32Window::Win32Window(const Vec2i size, const char* name)
	...
	// returns the HINSTANCE as void*
	void* Win32Window::getNativeInstance()
	{
		return hInstance;
	}

	// returns the HWND as void*
	void* Win32Window::getNativeHandle()
	{
		return hWnd;
	}

	Vec2i Win32Window::getWindowSize() const
	{
		return windowSize;
	}
	...
	bool Win32Window::isFramebufferResized() const
	{
		return resized;
	}

	void Win32Window::setFramebufferResized(bool resize)
	{
		resized = resize;
	}
	...
	Vec2i Win32Window::screenSpaceToWindowSpace(Vec2i& position) const
	{
	...
	bool Win32Window::getKeyPressed(int key) const
	{
		return ((GetKeyState(key) & 0x8000) && focus );
	}

	bool Win32Window::mouseButton(signed char buttonIndex, signed char type) const
	{
		return internalMouseStorage.buttons[buttonIndex * 2 + type];
	}

	signed char Win32Window::mouseWheel() const
	{
		return internalMouseStorage.scrollCount;
	}

	Vec2i Win32Window::mousePosition()
	{
		POINT point;
		GetCursorPos(&point);
	...
	void Win32Window::resizeWindow(HWND hWnd,UINT uMsg)
	{
		Win32Window *windowPtr = reinterpret_cast<Win32Window*>(GetWindowLongPtr(hWnd, GWLP_USERDATA));
