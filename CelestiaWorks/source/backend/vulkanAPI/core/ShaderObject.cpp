...
static const std::vector<uint32_t> getDefaultShaders(celestia::ShaderType shader,celestia::RenderGroup type)
{
	if (shader == celestia::ShaderType::VERTEX_SHADER)
	{
		...
		{
			const std::vector<uint32_t> vertexBinaryText(batchTextVertexShader, batchTextVertexShader + sizeof(batchTextVertexShader) / sizeof(uint32_t));
		
			return vertexBinaryText;
		}
		else if (type == celestia::RenderGroup::SPRITE_BATCH)
		{
			const std::vector<uint32_t> vertexBinaryBatch(batchVertexShader, batchVertexShader + sizeof(batchVertexShader) / sizeof(uint32_t));
			return vertexBinaryBatch;
		}
		else if (type == celestia::RenderGroup::BASIC)
		{
			const std::vector<uint32_t> vertexBinaryBasic(basicVertexShader, basicVertexShader + sizeof(basicVertexShader) / sizeof(uint32_t));
	...
	if (type == celestia::RenderGroup::TEXT_BATCH)
	{
		const std::vector<uint32_t> fragmentBinaryText(batchTextFragmentShader, batchTextFragmentShader + sizeof(batchTextFragmentShader) / sizeof(uint32_t));
		return fragmentBinaryText;
	}
	else if (type == celestia::RenderGroup::SPRITE_BATCH)
	{
		const std::vector<uint32_t> fragmentBinaryBatch(batchFragmentShader, batchFragmentShader + sizeof(batchFragmentShader) / sizeof(uint32_t));
		return fragmentBinaryBatch;
	}

	else if (type == celestia::RenderGroup::BASIC)
	{
		const std::vector<uint32_t> fragmentBinaryBasic(basicFragmentShader, basicFragmentShader + sizeof(basicFragmentShader) / sizeof(uint32_t));
