...
std::unique_ptr<celestia::Font_t> celestia::FontReader::createFont(const char* filepath, uint8_t fontSize)
{
    ...
    {
        std::cerr << "ERROR: The font file could be opened and read, but it appears that its font format is unsupported!\n";
        return nullptr;
    }
    else if (error)
    {
        std::cerr << "ERROR: the font file could not be opened or read! The filepath might be wrong or something else is bonked...\n";
        return nullptr;
    }
    ...
    VkDeviceSize textureSize = static_cast<VkDeviceSize>(bmpWidth) * font->bmpHeigth;  // * pixelSize; // tätä ei tarvi koska pixelin koko on valmiiksi jo 1
