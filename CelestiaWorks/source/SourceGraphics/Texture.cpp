...
celestia::Texture::Texture(const vk::RawTexture* rawTexture, Vec2i textureSize, bool ownsTexture)
...
celestia::Texture::~Texture()
{
	...
	{
		vk::Image::deleteTextureImage(*pixels);
	}
}

bool celestia::Texture::loadTexture(const char* filepath)
{
	pixels = std::make_unique<vk::RawTexture>(vk::Image::createTextureImage(filepath, size));
...
const celestia::vk::RawTexture* celestia::Texture::getRawTexturePtr() const
{
