#include "IndexBufferGenerator.hpp"
#include <stdexcept>
...
namespace celestia
{
	...
	std::vector<uint16_t> createIndexBufferForDrawType(size_t vertexCount, DrawType type)
	{
		...
		switch(type)
		{
		case DrawType::QUAD: generateQuadIndices(indices, vertexCount / 4, offset); break;
		case DrawType::TRIANGLE: generateTriangleIndices(indices, vertexCount / 3, offset); break;
		case DrawType::LINES: generateLineIndices(indices, vertexCount / 2, offset); break;
		case DrawType::DOTS: generateDotIndices(indices, vertexCount, offset); break;
		default: throw std::invalid_argument("Unsupported DrawType");
		}
