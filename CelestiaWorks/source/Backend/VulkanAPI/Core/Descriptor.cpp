  ...
  uint32_t binding, DescriptorType type, VkShaderStageFlags shaderStages, uint32_t descriptorCount)
{
	...
	if(type == DescriptorType::COMBINED_IMAGE_SAMPLER || type == DescriptorType::STORAGE_IMAGE ||
	   type == DescriptorType::BINDLESS_IMAGE)
	{
  ...
  uint32_t binding, const VkImageView* imageViews, VkSampler sampler, uint32_t count, uint32_t frameIndex)
{
	...
	if(bindingInfo->type != DescriptorType::COMBINED_IMAGE_SAMPLER &&
	   bindingInfo->type != DescriptorType::STORAGE_IMAGE && bindingInfo->type != DescriptorType::BINDLESS_IMAGE)
	{
	...
	for(uint32_t i = 0; i < count; i++)
	{
		...
		bindingInfo->imageInfos[i].imageView =
		  (imageViews[i] != VK_NULL_HANDLE) ? imageViews[i] : imageViews[0]; // fallback to first image
	}
...
void celestia::Descriptor::flushWrites()
{
	if(!pendingWrites.empty())
	{
		vkUpdateDescriptorSets(
		  Device::context.device, static_cast<uint32_t>(pendingWrites.size()), pendingWrites.data(), 0, nullptr);
		pendingWrites.clear();
	}
}
...
VkDescriptorType celestia::Descriptor::toVkType(DescriptorType type)
{
	...
	case DescriptorType::BINDLESS_IMAGE: return VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
