...
namespace celestia
{
	...
	public:
		...
		void addBinding(
		  uint32_t binding, DescriptorType type, VkShaderStageFlags shaderStages, uint32_t descriptorCount = 1);
		...
		void updateTexture(
		  uint32_t binding, const VkImageView* imageViews, VkSampler sampler, uint32_t count, uint32_t frameIndex = 0);

		// Update buffer binding for a specific frame
		void
		updateBuffer(uint32_t binding, VkBuffer* buffers, VkDeviceSize size, uint32_t count, uint32_t frameIndex = 0);
