...
void celestia::SwapChain::createSyncObjects()
{
	...
	for(size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
	{
		if(vkCreateSemaphore(Device::context.device, &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) !=
			 VK_SUCCESS ||
		   vkCreateSemaphore(Device::context.device, &semaphoreInfo, nullptr, &renderFinishedSemaphores[i]) !=
			 VK_SUCCESS ||
...
VkExtent2D celestia::SwapChain::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities)
{
	...
	else
	{
		...
		VkExtent2D actualExtent = {static_cast<uint32_t>(winSize.x), static_cast<uint32_t>(winSize.y)};
