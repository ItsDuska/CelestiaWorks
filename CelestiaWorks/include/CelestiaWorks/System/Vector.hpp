...
namespace celestia
{
    ...
    template <typename T> constexpr Vec<4, T> calc(Vec<4, T> const& a, Vec<4, T> const& b)
    {
        return Vec<4, T>(a.x * b.x, a.y * b.y, a.z * b.z, a.w * b.w);
    }

    template <typename T> constexpr Vec<4, T> calcAdd(Vec<4, T> const& a, Vec<4, T> const& b)
    {
        return Vec<4, T>(a.x + b.x, a.y + b.y, a.z + b.z, a.w + b.w);
    }

    // vec2
    template <typename T> struct CELESTIA_WORKS Vec<2, T>
    {
        T x, y;
        ...
        template <typename U> constexpr Vec(U _x, U _y) : x(static_cast<T>(_x)), y(static_cast<T>(_y))
        {}
    ...
    // vec3
    template <typename T> struct CELESTIA_WORKS Vec<3, T>
    {
        union
        {
            struct
        ...
        template <typename U>
        constexpr Vec(U _x, U _y, U _z) : x(static_cast<T>(_x)), y(static_cast<T>(_y)), z(static_cast<T>(_z))
        {}
    ...
    // vec4
    template <typename T> struct CELESTIA_WORKS Vec<4, T>
    {
        union
        {
            struct
        ...
        {}
        // <Constructor>
        constexpr Vec(Vec<4, T> const& value) : x(value.x), y(value.y), z(value.z), w(value.w)
        {}
        constexpr explicit Vec(T value) : x(value), y(value), z(value.z), w(value.w)
        {}
